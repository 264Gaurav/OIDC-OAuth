...
```text
Platform
└── Partner Group / Partner Tenant
    ├── Partner Admin(s)
    ├── Partner User(s)
    ├── Partner Resources / Modules
    │
    └── Customer Group / Customer Tenant
        ├── Customer Admin(s)
        ├── Customer User(s)
        └── Customer Resources
            ├── Devices
...
A **Customer Group belongs to exactly one Partner Group**. A user may have membership in one or more scopes only where explicitly supported by policy; the effective permissions for a request must always be calculated from the user's current role, membership and resource scope.
...
SuperAdmin access is a **platform scope**, not a Partner scope. A SuperAdmin may cross Partner boundaries only because the role is explicitly authorized at the platform level.
...
| Role | Primary scope | Can manage | Cross-tenant access |
|---|---|---|---|
| SuperAdmin | Platform | Partners, Partner Admins, platform-scoped resources/modules | Yes, by explicit platform authority |
| PartnerAdmin | One Partner | Partner Users, Customers, Customer Admins/Users, Partner resources/modules | No, outside assigned Partner |
| PartnerUser | One Partner | Only explicitly assigned Partner resources/modules | No |
| CustomerAdmin | One Customer Group | Customer Users, devices, device buckets, Customer resources | No |
| CustomerUser | One Customer Group | Devices/device buckets allowed by assigned permissions | No |
...
10. SuperAdmin cross-tenant access is an explicit platform capability and must be audited.
...
A role assignment should be represented as a relationship that includes scope. Conceptually:
...
- A SuperAdmin can provision/manage Partner Groups according to platform policy.
...
# 36. Standards and Primary References
